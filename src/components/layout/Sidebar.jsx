    ...
    <>
      ...
      <aside
        ...
        <div style={{ borderBottom: '0.5px solid rgba(255,255,255,0.06)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '13px 13px 8px' }}>
            ...
            <button
              onClick={onClose}
              style={{ background: 'none', border: 'none', color: '#6b7789', fontSize: 18, padding: 2, cursor: 'pointer', display: 'none' }}
              className="mobile-close-btn"
              aria-label="Close menu">
              <i className="ti ti-x" aria-hidden="true"/>
            </button>
          </div>
          <DomainSwitcher onNavigate={() => navigate('domain')} />
        </div>
            ...
            return (
              <button key={item.id} onClick={() => navigate(item.id)}
                ...
                style={{
                  ...
                  padding: '7px 11px', margin: '1px 6px', borderRadius: 7,
                  fontSize: 11.5, cursor: 'pointer', textAlign: 'left',
                  width: 'calc(100% - 12px)', transition: 'all .12s', whiteSpace: 'nowrap',
        ...
        <div style={{ padding: '10px', borderTop: '0.5px solid rgba(255,255,255,0.06)' }}>
          ...
          <button onClick={handleSignOut} disabled={signingOut} aria-label="Sign out"
            style={{
              ...
              fontFamily: 'Syne,sans-serif', transition: 'all .15s', opacity: signingOut ? 0.6 : 1,
    ...
    <div style={{ position: 'relative', margin: '0 8px 8px' }}>
      ...
      {open && domains.length > 1 && (
        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: '#161c2a', border: '0.5px solid rgba(255,255,255,0.1)', borderRadius: 7, zIndex: 100, marginTop: 3, overflow: 'hidden' }}>
          {domains.map(d => (
            <button key={d.id} onClick={() => { switchDomain(d.id); setOpen(false) }}
              ...
              <span style={{ fontFamily: 'IBM Plex Mono,monospace', fontSize: 10, color: d.id === activeDomain.id ? '#4fa6ff' : '#6b7789', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.name}</span>
